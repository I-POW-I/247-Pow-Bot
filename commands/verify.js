...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (sub === 'setup') {
      ...
      const desc  = interaction.options.getString('description') || 'Click the button below to verify.';

      // Check bot can assign this role
      const botMember = await guild.members.fetchMe();
      if (botMember.roles.highest.position <= role.position) {
        return interaction.reply({
          content: `❌ 24/7 POW Bot cannot assign **${role.name}** because it is equal to or above the bot’s highest role. Please move the bot role above it in Server Settings → Roles.`,
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(desc)
        .setColor(0x7b8cff)
        .setFooter({ text: '24/7 POW Bot • Verification' });
      ...
      return interaction.reply({
        content: `✅ Verification is now live. Members can click the button below to receive the **${role.name}** role.`,
        flags: [MessageFlags.Ephemeral],
      });
    }
  },
