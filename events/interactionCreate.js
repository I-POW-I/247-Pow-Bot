...
async function joinChannel(targetChannel, guild, member, client, interaction) {
  ...
  if (existingConn) {
    if (HEALTHY.includes(existingConn.state.status)) {
      ...
      return interaction.reply({
        content: `⚠️ 24/7 POW Bot is already connected to **${entry?.channelName || 'a voice channel'}**. Use Leave first to disconnect it cleanly.`,
  ...
  try {
    ...
    return interaction.reply({
      content: '❌ 24/7 POW Bot could not join that channel. Please check that it has permission to connect and try again.',
      flags: [MessageFlags.Ephemeral],
    });
  }
}
  ...
  async execute(interaction, client) {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      try {
        ...
        const reply = { content: '❌ The request could not be completed. Please try again.', flags: [MessageFlags.Ephemeral] };
    ...
    if (interaction.isUserSelectMenu() && interaction.customId === 'bot_lookup_user') {
      ...
      if (!member) {
        return interaction.reply({ content: '❌ 24/7 POW Bot could not find that member. Please try again.', flags: [MessageFlags.Ephemeral] });
      }
      return interaction.reply({
    ...
    if (interaction.customId === 'bot_join') {
      if (!isAdmin) {
        return interaction.reply({ content: '🚫 You need **Manage Server** permission to use this control.', flags: [MessageFlags.Ephemeral] });
      }
      ...
      return interaction.reply({
        content:    'You are not in a voice channel right now. Choose one below to continue.',
    ...
    if (interaction.customId === 'bot_leave') {
      ...
      const entry = store.getEntry(guild.id);
      if (!conn && !entry) return interaction.reply({ content: '❌ 24/7 POW Bot is not currently connected to a voice channel.', flags: [MessageFlags.Ephemeral] });
